...
export async function GET(request: Request) {
  console.log('🚀 LIVE EMAIL CRON TRIGGERED');
  ...
  try {
    ...
    // Check if it's Monday, November 24, 2025
    ...
    if (dayOfWeek !== 'Monday') {
      console.log(`⏭️ Skipping - Current day: ${dayOfWeek}, Expected: Monday`);
      return NextResponse.json({
        success: true,
    ...
    console.log('✅ Time check passed - Executing on Monday at 8 AM NY time');

    const startTime = Date.now();

    console.log('📧 Starting live email batch (ONE-TIME SEND)');
    ...
    const response = {
      success: true,
      message: 'Live emails sent (ONE-TIME)',
    ...
    console.log('✅ Live emails completed:', JSON.stringify(response, null, 2));

    return NextResponse.json(response);
  } catch (error) {
    console.error('❌ Live emails failed:', error);
...
async function sendEmails(emails: string[]) {
  let sent = 0;
  let failed = 0;
  const batchSize = 10; // Send in batches
